...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      password,
      options: {
        data: { role: "customer", full_name: fullName, phone, referral_code: referralCode.trim() },
      },
    });
    ...
    router.push("/klant/bevestig-email");
  }

  return (
    <div className="flex flex-col h-full">
      <NavBar onBack={() => router.push("/klant/login")} />
      <form onSubmit={handleSubmit} className="px-5 pt-2 flex-1 overflow-y-auto no-scrollbar flex flex-col">
        <div className="text-[28px] font-bold tracking-[-0.02em]">Account aanmaken</div>
        <div className="text-[15px] text-text-secondary mt-1.5">Binnen een minuut geregeld.</div>
