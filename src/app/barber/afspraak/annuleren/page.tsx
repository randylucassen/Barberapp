...
import { BARBER_CANCEL_REASONS } from "@/lib/mock-data";
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 pt-4 flex-1">
        ...
        <div className="mt-2 bg-error-soft rounded-md px-4 py-3 text-[13px] text-error-text leading-[19px]">
          De klant ontvangt direct bericht van de annulering en krijgt zijn betaling volledig terug.
        </div>
        {error && (
          <div className="mt-2 bg-error-soft rounded-md px-4 py-3 text-[13px] text-error-text leading-[19px]">
            {error}
        ...
        <div className="mt-4">
          {BARBER_CANCEL_REASONS.map((r) => (
            <Row
              key={r}
              onClick={() => setReason(r)}
              title={r}
              right={<Radio checked={reason === r} onChange={() => setReason(r)} />}
            />
          ))}
        </div>
        {isOther && (
          <div className="mt-3">
            <Input
              label="Reden"
              placeholder="Bijv. klant belt niet terug"
              value={customReason}
      ...
      <div className="px-5 pt-3 pb-2 flex flex-col gap-2">
        <Button full variant="secondary" disabled={!canSubmit || submitting} onClick={handleCancel}>
          {submitting ? "Bezig…" : "Annuleer afspraak"}
        </Button>
